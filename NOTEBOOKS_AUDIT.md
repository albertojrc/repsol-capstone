...
| `11_mini_demand_model.ipynb` | Removed upstream | A different, since-removed file. Superseded by what is now `11_mini_trend_regulation_model.ipynb` below (renumbered from 12 on 2026-06-25, see `memory.md`'s dated entry) -- the two should not be confused just because they now share a leading number. |
| `11_mini_trend_regulation_model.ipynb` | Mini regulation model | Narrative regulation/trend scenario model, independent of the main per-region pipeline. Not affected by the translation bugs (no `'Trend'`/`gasolina` literals found). Its own scenario forecasts are not reconciled with `forecast_24m_selected.csv` -- the two should not be confused for the same deliverable. Renumbered from `12_mini_trend_regulation_model.ipynb` on 2026-06-25 to close the gap left by the row above. |
| `12_business_interpretation_and_recommendations.ipynb` | Business interpretation; fixed and re-verified | Reads production outputs, restates selected results by target, shows the training-only SARIMA grid, generates regional train/validation/forecast plots, and documents model limitations, feature interpretation, internal Repsol data needs, and recommendations. Fixed the stale SARIMA-acceptance columns and the `?` mojibake; re-executed to regenerate all 5 regional figures, which were previously a day stale relative to the final model selection. Renumbered from `13_business_interpretation_and_recommendations.ipynb` on 2026-06-25; its own `13_business_*.png` figure outputs were renamed to `12_business_*.png` to match. |
...
and 12 reflect the current selection (10.1 was consolidated into 10, and the
business-interpretation notebook was renumbered from 13 to 12 -- see the
